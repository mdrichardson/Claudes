(function (root, factory) {
  ...
  var PLAN_MODE_BANNER = /\x1b\[7m[^\x1b]*Plan(?:[^\x1b]{0,40}?Mode|[^\x1b]*\x1b\[(?:0|27)m[\s\S]{0,40}?Mode)/i;
  ...
  function findExecuteIdx(state) {
    ...
    for (var i = 0; i < state.steps.length; i++) {
      if (state.steps[i] && state.steps[i].id === 'anchor-execute') return i;
    }
    return 1;
  }
  ...
  function applyPlanExit(state) {
    ...
    if (Array.isArray(state.steps) && state.steps.length > 0 && state.steps[0]) {
      state.steps[0].complete = true;
    }
    state.currentIdx = findExecuteIdx(state);
    return true;
  }
